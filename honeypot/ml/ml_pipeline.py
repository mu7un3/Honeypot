...
# Ensure the parent honeypot package is on sys.path for config import
...
try:
    ...
    ML_AVAILABLE = False
    logging.warning(
        "scikit-learn not available — using rule-based fallback models.")
...
def _verify_hash(path: Path) -> bool:
    ...
    if not hash_file.exists():
        logging.warning(
            f"No integrity file for {path.name} — skipping verification.")
        return True  # Warn but don't hard-fail on first use
    expected = hash_file.read_text().strip()
    actual = _file_sha256(path)
    if expected != actual:
        logging.error(
            f"Integrity check FAILED for {path.name}: file may have been tampered with.")
        return False
    return True
...
def rule_based_threat_score(attack: dict) -> float:
    ...
    if "Scan" in attack_type:
        score += 1
    if attack.get("info", {}).get("malware_family"):
        score += 3
...
def threat_recommendations(attack: dict, score: float) -> list[str]:
    ...
    port = attack.get("honeypot_port", 0)
    if score >= 6:
        recs += ["Block attacker IP in firewall",
                 "Enable rate limiting on affected service"]
    if port == 22:
        recs += ["Enable fail2ban for SSH",
                 "Disable password auth — use SSH keys only"]
    elif port in (21, 23):
        recs += ["Disable legacy protocols (FTP/Telnet)",
                 "Use SFTP/SSH instead"]
...
class HoneypotML:
    def __init__(self) -> None:
        ...
        # stored for distribution drift detection
        self._scaler_mean:    np.ndarray | None = None

    # ── Training ───────────────────────────────────────────────────────────────

    @staticmethod
    def _load_csv(path: str) -> tuple[list[dict], list[str], list[float]]:
        """Load flat CSV training data and return (feature_rows, labels, threat_scores)."""
        X_raw: list[dict] = []
        y_classify: list[str] = []
        y_threat: list[float] = []
        ...
        with open(path, "r", encoding="utf-8") as f:
            ...
            for row in reader:
                features = {
                    "port":                float(row.get("port", 0)),
                    "port_commonality":    float(row.get("port_commonality", 0)),
                    "requests_per_second": float(row.get("requests_per_second", 0)),
                    "unique_payloads":     float(row.get("unique_payloads", 0)),
                    "duration":            float(row.get("duration", row.get("duration_seconds", 0))),
                    "is_scanning":         int(float(row.get("is_scanning", 0))),
                    "is_exploit":          int(float(row.get("is_exploit", 0))),
                    "is_bruteforce":       int(float(row.get("is_bruteforce", 0))),
                    "time_of_day":         float(row.get("time_of_day", 12)),
                    "day_of_week":         float(row.get("day_of_week", 0)),
                    "has_geo":             int(float(row.get("has_geo", 0))),
                    "indicator_count":     float(row.get("indicator_count", 0)),
                    "has_malware":         int(float(row.get("has_malware", 0))),
                }
                X_raw.append(features)
                y_classify.append(row.get("attack_type", "Unknown Attack"))
                y_threat.append(
                    float(row.get("threat_level", row.get("threat_score", 5.0))))

        return X_raw, y_classify, y_threat

    @staticmethod
    def _load_jsonl(path: str) -> tuple[list[dict], list[str], list[float]]:
        """Load JSONL training data and return (feature_rows, labels, threat_scores)."""
        attacks: list[dict] = []
        with open(path, "r", encoding="utf-8") as f:
        ...
        y_classify = [a.get("attack_type") or rule_based_classify(a)
                      for a in attacks]
        y_threat = [a.get("threat_score", rule_based_threat_score(a))
                    for a in attacks]
        return X_raw, y_classify, y_threat
    ...
    def train(self, training_data_path: str) -> None:
        ...
        try:
            if training_data_path.endswith(".csv"):
                X_raw, y_classify, y_threat = self._load_csv(
                    training_data_path)
            else:
                X_raw, y_classify, y_threat = self._load_jsonl(
                    training_data_path)
        ...
        print(f"Loaded {len(X_raw)} attack records.")
        ...
        self._scaler_mean = self.scaler.mean_.copy()  # type: ignore[union-attr]
        ...
        print(classification_report(y_cls_test, y_pred,
                                    target_names=self.label_encoder.classes_))  # type: ignore[union-attr]
        ...
        self.clusterer = KMeans(n_clusters=ML_N_CLUSTERS,
    ...
    def predict_attack_type(self, attack: dict) -> dict:
        ...
        if not ML_AVAILABLE or self.classifier is None or self.label_encoder is None:
        ...
        return {
            "predicted_type": self.label_encoder.inverse_transform([prediction])[0],  # type: ignore[union-attr]
            "confidence":     float(max(probabilities)),
            "all_probabilities": {
                cls: float(prob)
                for cls, prob in zip(self.label_encoder.classes_, probabilities)  # type: ignore[union-attr]
            },
            "model": "RandomForest",
        }
    ...
    def detect_anomaly(self, attack: dict) -> dict:
        ...
        anomaly_score = abs(
            float(self.anomaly_detector.score_samples(X_scaled)[0]))
    ...
    def score_threat(self, attack: dict) -> dict:
        ...
        if ML_AVAILABLE and self.threat_scorer and self.is_trained:
            X_scaled = self._scale(attack)
            ml_score = float(
                np.clip(self.threat_scorer.predict(X_scaled)[0], 0, 10))
    ...
    def save_models(self, path: Path = MODELS_DIR) -> None:
        ...
        if self.scaler is not None and hasattr(self.scaler, 'mean_'):
            np.save(str(path / "scaler_mean.npy"), self.scaler.mean_)  # type: ignore[arg-type]
        ...
        try:
            for filename, attr in model_files.items():
                ...
                if not _verify_hash(file_path):
                    raise ValueError(
                        f"Integrity check failed for {filename} — aborting load.")
    ...
    if len(sys.argv) > 1:
        ...
        if cmd == "train":
            data_file = sys.argv[2] if len(
                sys.argv) > 2 else "honeypot_synthetic.jsonl"
            ml.train(data_file)
        ...
        print("Usage:")
        print(
            "  python ml_pipeline.py train [data_file]   Train models on JSONL data")
